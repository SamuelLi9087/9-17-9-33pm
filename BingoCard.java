...
public class BingoCard {
	...
	public BingoCard(int seed, int seedCards) {
		...
		bingoImages = new BufferedImage(2550,3330,BufferedImage.TYPE_INT_ARGB);
	...
	/*public void callBingoBallTest() {
		...
		for(SNDWIE x: bingoCards) {
			x.checkBall(oldasf.get(0));
		}
	}
	*/
	...
	public boolean checkWinners() {
		...
		//first column
		else if(card[0][0]==0 && card[0][1]==0 &&card[0][2]==0 &&card[0][3]==0 &&card[0][4]==0) {
			didIwin = true;
		}
		//second
		else if(card[1][0]==0 && card[1][1]==0 && card[1][2]==0 && card[1][3]==0 &&card[1][4]==0) {
